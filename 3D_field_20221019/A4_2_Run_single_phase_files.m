disp('Now running the upscaling simmulation files')
disp(' ')
%Now we run the upscaling simulations files. This involves a lot of error
%checking and re-running, since if we any disconnected phases in kr, the
%simulation essentially fails to converge. If this happens, we have to
%identify it, remove the kr point from the simulation runs (since it has kr = 0)
%and re run the simulation files.
    ...
    for jjj = 1:N_hom_subs_x
        for kkk = 1:N_hom_subs_y
            ...
            while (isempty(k))
                ...
                %file.
                disp(['Error in upscaling horizontal subvolume #', int2str(iii),',',int2str(jjj),',', int2str(kkk)]);
                attemp_no = attemp_no+1;

                count1 = 0;
                count2 = 0;

                k2 = strfind(cmdout,'Opened GRID scratchfile on unit 19');
                [~,n] = size(k2);
                n = n-1;
                ...
                elseif (n == 0)
                    disp(['Horiztonal simulation didn''t run for vol #', int2str(iii),',',int2str(jjj),',',int2str(kkk)]);
                else
                ...
                fprintf(fid, '%s\r\n', 'WSRF GRID TIME');
                fprintf(fid, '%s\r\n', 'WSRF WELL TIME');
                fprintf(fid, '%s\r\n', 'OUTSRF GRID PRES SO SW PCOW PCWSCL VISO VISW MASDENO KRO KRW FLUXRC');
                fprintf(fid, '%s\r\n', 'OUTSRF WELLRATES');
                fprintf(fid, '%s\r\n', '');
                fprintf(fid, '%s\r\n', '***************************************************************************');
                fprintf(fid, '%s\r\n', '**');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%            
           ...
           disp("running horz file number 2")
            ...
            [status, cmdout] = system([exe_path_run, ' -f ', filename, '.dat -wait']);
            %[status, cmdout] = system([exe_path_run, ' -f ', filename, '.dat '],'-echo');
            k = strfind(cmdout,'End of Simulation: Normal Termination');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %-------------------------------------------------------------------------------------------------------------------------------------------Vertical upscaling--------------
            %Now run the vertical case. This is the same as the horiztonal
            %case.
            ...
            while (isempty(k))
                ...
                elseif (n == 0)
                    ...
                    disp(['Vertical simulation didn''t run for vol #', int2str(iii),',',int2str(jjj), ',', int2str(kkk)]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                %Generate vertical upscaling file.
            ...
            save('subvol_struct.mat', 'subvol_struct', '-v7.3');
