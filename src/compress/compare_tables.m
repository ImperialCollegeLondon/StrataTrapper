function err = compare_tables(tables_1,tables_2,fit_parametric,fit_total_mobility,...
...
transform = @(table) to_features(table,fit_parametric,fit_total_mobility,num_principal_components);

for param_id = 1:size(tables_1,1)
    for dir=1:3

        feat_dir_1 = transform(tables_1(param_id,dir));
        feat_dir_full_1 = feat_dir_1(:,tables_1(param_id,dir).mapping);

        feat_dir_2 = transform(tables_2(param_id,dir));
