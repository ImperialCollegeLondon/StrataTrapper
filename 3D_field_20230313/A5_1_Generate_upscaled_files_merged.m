...
for qq = 1:N_Qg
 ...
 %%%% THIS WILL HAVE TO CHANGE DEPENDING ON HPC
    ...
    fprintf(fid, '%s\r\n', '25 /');
    fprintf(fid, '%s\r\n', '');
    fprintf(fid, '%s\r\n', 'LGR'); %set options for local grid refinement

    %MAXLGR (max no of LGRs)
    %MAXCLS (max no of cells in each LGR)
    %MCOARS (max no of amalgamated cells)
    % MAALG (max no of LGR amalgamations)
    % LSTACK 

%%%% THIS WILL HAVE TO BE ADDED BACK IN
   ...
   fprintf(fid, '%s\r\n', 'PERMX');
    ...
    % name of LGR
    % Lower i coordinate of box in parent grid
    % higher i coordinate of box in parent grid
    % low j
    % high j
    % low k 
    % high k
    % no of refined cells along x direction
    % "" in y direction
    % "" in z direction 
    % max no of wells this LGR will contain

    %we want our grid refinement to be all across the first layer,so from
    %X=1-1, Y= 1-N_hom_subs_y, Z = 1-N_hom_subs_z
   
    fprintf(fid, '%s\r\n',  [' ''LGR'' ',' 1 1 1 ',int2str(N_hom_subs_y),' 1 ',int2str(N_hom_subs_z),' ', int2str(Nx_up*1),' ', int2str(Ny_up*N_hom_subs_y),'  ' ,int2str(Nz_up*N_hom_subs_z),' ' ,int2str(Nz), ' / ']);
   % fprintf(fid, '%s\r\n', 'LGRCOPY');
...
%     end
   ...
   %THIS IS SECONDARY GRID REFINEMENT
    ...
    s_zone_width = ceil((N_hom_subs_x-2)*0.5); %60% of the remaining cells 
    s_zone_width_2 = N_hom_subs_x-2-s_zone_width;
    
    for i = 1:LGR_s_zones
        fprintf(fid, '%s\r\n', 'CARFIN');
        
        if (i == 1) %zone 1 - 60% of remaining grid blocks will be refined by factor of 8
            
            fprintf(fid, '%s\r\n',  [' ''LGR' ,int2str(i+1),''' ', int2str(2),' ', int2str(2+s_zone_width-1), ' ', ' 1 ',int2str(N_hom_subs_y),' ', ' 1 ',int2str(N_hom_subs_z),' ', int2str(s_zone_width*Nx_up), ' ' , int2str(N_hom_subs_y*Ny_up), ' ', int2str(N_hom_subs_z*Nz_up), ' ',int2str(Nz), ' / ']);
            ...
            fprintf(fid, '%s\r\n',  [' ''LGR' ,int2str(i+1),''' ', int2str(2+s_zone_width),' ', int2str(2+s_zone_width-1+s_zone_width_2), ' ', ' 1 ',int2str(N_hom_subs_y),' ', ' 1 ',int2str(N_hom_subs_z),' ', int2str(s_zone_width_2*Nx_up), ' ' , int2str(N_hom_subs_y*Ny_up), ' ', int2str(N_hom_subs_z*Nz_up), ' ',int2str(Nz), ' / ']);
                fprintf(fid, '%s\r\n', 'LGRCOPY');
 ...
 % X Y Z
    ...
    % we want grid refinemtn from NX_MAX-NX_MAX, y whole, z whole
    ...
    for iii = 1:N_hom_subs_z %loop through each coarse grid cell
        for i = 1:Nz_sub
            for kkk = 1:N_hom_subs_y
                for k = 1:Ny_sub
                    for jjj = N_hom_subs_x:N_hom_subs_x %this is at producer boundary, so x = max
                        for j= 1:Nx_sub
...
%%%%%THIS MAY HAVE TO CHANGE WITH OUR GRID REFINEMENT??? NOT SURE

%%%%% ALSO NEED TO ADD THE IN DOMAIN FUNCTIONS BACK IN
    
    A5_4_write_pcfunctions_include_files_v3
...
%     %@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  fprintf(fid, '%s\r\n', 'FWIT');
    ...
    fprintf(fid, '%s\r\n',  'LBPR');
    fprintf(fid, '%s\r\n',  [' ''LGR'' ']);
    fprintf(fid, '%s\r\n',  [ int2str(1),' ', int2str(ceil(Ny/2)),' ', int2str(1), ' / ']);
    fprintf(fid, '%s\r\n',  [' ''LGR1'' ']);
    fprintf(fid, '%s\r\n',  [ int2str(Nx_up),' ', int2str(ceil(Ny/2)),' ', int2str(1), ' / ']);
    fprintf(fid, '%s\r\n',  '/');
    fprintf(fid, '%s\r\n', '');  
...
% THIS WILL HAVE TO BE UPDATED IF SIMULATOR DOESNT RUN
    ...
    for ii  = 1:1 %we only want 1 well, well_perforations_fine:well_perforations_fine:Nz
        count = count + 1;
        fprintf(fid, '%s\r\n', ['W', int2str(count),' G', ' ''LGR''  ', ' 1	 ', int2str(ceil(Ny/2)), ' 1* OIL	/' ]);
    end
    for ii  = 1:1 %we only want 1 well well_perforations_fine:well_perforations_fine:final_refined_Nz
        count = count + 1;
        fprintf(fid, '%s\r\n', ['W', int2str(count),' G  ',final_LGR ,	 int2str(final_refined_cell),' ', int2str(ceil(Ny/2)),'	 1* LIQ	/' ]);
    end
    fprintf(fid, '%s\r\n',  ' ');
    fprintf(fid, '%s\r\n', '/');
    fprintf(fid, '%s\r\n',  ' ');
    ...
    for ii  = 1:1 %well_perforations_fine:well_perforations_fine:Nz
        count = count + 1;
        fprintf(fid, '%s\r\n', ['W', int2str(count),' ''LGR'' ', ' 1 ',int2str(ceil(Ny/2)),' 1 ',int2str(Nz) ,' OPEN	1*	1*	0.00002	1*	0	0	Z	1*	/'  ]);
    end
    %start = count;
   % count = 0;
    for ii  = 1:1 %well_perforations_fine:well_perforations_fine:final_refined_Nz
        count = count + 1;
                fprintf(fid, '%s\r\n', ['W', int2str(count),' ',final_LGR,' ', int2str(final_refined_cell),' ',int2str(ceil(Ny/2)),' 1 ',int2str(Nz) ,' OPEN	1*	1*	0.00002	1*	0	0	Z	1*	/'  ]);
