# StrataTrapper agentic vibe-coding guide

Many parts of this document is generated by an LLM. Use with caution and always verify against the source code and documentation. The generated content may contain inaccuracies or outdated information.
...
**Author**: GitHub Copilot <github-copilot@users.noreply.github.com>
**Co-author**: (from `git config user.name` and `git config user.email`)
**Format**: Simple (description + co-author trailer, no GPG signing)
...
- `scope`: Feature or subsystem affected (consult `.vscode/settings.json` for approved scopes)
  - Examples: `compress`, `export`, `matlab`, `options`, `plot`

- `emoji`: Standard gitmoji
  - ✨ new feature
  - 🐛 bug fix
  - ✅ tests
  - ♻️ refactor
...
- `Co-authored-by`: Name and email of actual human developer (from git config)
...
Implements zeroth iteration of compression pipeline per design.md:
- Identity mapping (no compression yet): n_compressed = n_original
- Full input/output validation using granular validator functions
- Zero reconstruction error for stub implementation
...
- **Co-author pattern**: Always include co-author trailer with human's name and email
- **Simple format**: No additional boilerplate lines
- **Scope validation**: Consult `.vscode/settings.json` or copilot-instructions for valid scopes
