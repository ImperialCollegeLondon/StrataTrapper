            ...
            for kk = 1:n_pts %n_pts is the number of saturation points we want to use 
                ...
                while (err > tol_sw) && (count < 1500)
                    ...
                    if (pc_minus > (max(max(max(subvol_struct(iii,jjj,kkk).peMat)))))
                        ...
                        subvol_struct(iii,jjj,kkk).invaded_mat_minus(:,:,:,kk) = check_box_connectivity(pc_minus, subvol_struct(iii,jjj,kkk).peMat, Nx_sub, Nz_sub, Ny_sub); %check_box_connectivity(pc, Pe_mat, Nx, Nz)
                        subvol_struct(iii,jjj,kkk).invaded_mat_mid(:,:,:,kk) = check_box_connectivity(pc_mid, subvol_struct(iii,jjj,kkk).peMat, Nx_sub, Nz_sub, Ny_sub); %check_box_connectivity(pc, Pe_mat, Nx, Nz)
                        subvol_struct(iii,jjj,kkk).invaded_mat_plus(:,:,:,kk) = check_box_connectivity(pc_plus, subvol_struct(iii,jjj,kkk).peMat, Nx_sub, Nz_sub, Ny_sub);
                ...
                if (kk > 1 && kk < n_pts && sw_mid < subvol_struct(iii,jjj,kkk).sw_upscaled(kk-1))
                    subvol_struct(iii,jjj,kkk).sw_upscaled(kk) = sw_use;
                end
               ...
               [subvol_struct(iii,jjj,kkk).kg_phase_connected(kk,1), ~] = check_axis_connectivity(subvol_struct(iii,jjj,kkk).kg_mat(:,:,:,kk), Nx_sub, Nz_sub,Ny_sub,tol_kr, 1, Ny_sub);
               % disp("done with direction 1")
               [subvol_struct(iii,jjj,kkk).kg_phase_connected(kk,2), ~] = check_axis_connectivity(subvol_struct(iii,jjj,kkk).kg_mat(:,:,:,kk), Nx_sub, Nz_sub,Ny_sub,tol_kr, 2, Nx_sub);
               % disp("done with direction 2")                
               [subvol_struct(iii,jjj,kkk).kg_phase_connected(kk,3), ~] = check_axis_connectivity(subvol_struct(iii,jjj,kkk).kg_mat(:,:,:,kk), Nx_sub, Nz_sub,Ny_sub,tol_kr, 3, Nz_sub);
               % disp("done with direction 3")
                
               % disp("moving on to kw")
               [subvol_struct(iii,jjj,kkk).kw_phase_connected(kk,1), ~] = check_axis_connectivity(subvol_struct(iii,jjj,kkk).kw_mat(:,:,:,kk), Nx_sub, Nz_sub,Ny_sub,tol_kr, 1, Ny_sub);
               %disp("done with direction 1")
               [subvol_struct(iii,jjj,kkk).kw_phase_connected(kk,2), ~] = check_axis_connectivity(subvol_struct(iii,jjj,kkk).kw_mat(:,:,:,kk), Nx_sub, Nz_sub,Ny_sub,tol_kr, 2, Nx_sub);
               % disp("done with direction 2") 
               [subvol_struct(iii,jjj,kkk).kw_phase_connected(kk,3), ~] = check_axis_connectivity(subvol_struct(iii,jjj,kkk).kw_mat(:,:,:,kk), Nx_sub, Nz_sub,Ny_sub,tol_kr, 3, Nz_sub);
               % disp("done with direction 3")
